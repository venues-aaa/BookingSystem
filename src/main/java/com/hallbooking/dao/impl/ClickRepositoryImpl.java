...
public class ClickRepositoryImpl implements ItemDao {
	...
	public List<Item> fetchItems(Item item) {
		...
		if (item.getType() != null && !item.getType().isEmpty()) {
			criteria = criteria.and(DBConstants.ITEM_TYPE).is(item.getType());
			hasCriteria = true;
		}

		// Add placeId filter if provided
		if (item.getPlaceId() != null && !item.getPlaceId().isEmpty()) {
			criteria = criteria.and(DBConstants.ITEM_PLACE_ID).is(item.getPlaceId());
			hasCriteria = true;
		}
	...
	public void updateItemDetails(Item itemDetails) throws JsonProcessingException {
		System.out.println("=== DAO: updateItemDetails called ===");

		Query updateQuery = new Query();
		// Handle both ObjectId format (24 hex chars) and custom string IDs like "hotel001"
		String itemId = itemDetails.getId();
		System.out.println("Item ID to update: " + itemId);

		if (itemId != null && itemId.matches("[0-9a-fA-F]{24}")) {
			// Valid MongoDB ObjectId format - convert to ObjectId
			System.out.println("Using ObjectId format");
			updateQuery.addCriteria(Criteria.where(DBConstants.ITEM_ID).is(new ObjectId(itemId)));
		} else {
			// Custom string ID - use as-is
			System.out.println("Using string ID format");
			updateQuery.addCriteria(Criteria.where(DBConstants.ITEM_ID).is(itemId));
		}
		...
		update.set(DBConstants.ITEM_STATUS, itemDetails.getStatus());
		update.set(DBConstants.ITEM_IMAGES, itemDetails.getImages());
		...
		var result = mongoTemplate.updateFirst(updateQuery, update, Item.class);
		System.out.println("MongoDB update result - Matched: " + result.getMatchedCount() + ", Modified: " + result.getModifiedCount());

		if (result.getMatchedCount() == 0) {
			System.err.println("WARNING: No document matched the query. Item ID may not exist: " + itemId);
		}

		System.out.println("=== DAO: updateItemDetails completed ===");
	}
	...
	public List<Item> filteredItems(ItemSearchCriteria itemSearchCriteria) {
		...
		if(null != itemSearchCriteria.getSeatingCapacity()) {
			criteria = firstCondition ? Criteria.where(DBConstants.BOOKING_QTY_AVAILABLE).gte(itemSearchCriteria.getSeatingCapacity()) : criteria.and(DBConstants.BOOKING_QTY_AVAILABLE).gte(itemSearchCriteria.getSeatingCapacity());
			firstCondition = false;
		}
